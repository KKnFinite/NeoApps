    ...
    def test_tunnel_reference_sections_and_hooks_with_or_without_notice(self):
        for active in (True, False):
            with self.subTest(active_refresh=active):
                ...
                lower = wrap.one('div', 'tunnel-lower-controls')
                self.assertIn(operations, lower.children)
                self.assertIn(wrap.one('section', 'tunnel-offset-panel'), lower.children)
                self.assertFalse(lower.findall('section', 'tunnel-bay-panel'))
