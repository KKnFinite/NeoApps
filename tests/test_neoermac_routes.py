...
class NeoErmacRoutesTest(unittest.TestCase):
    ...
    def test_building_lineup_page_renders_belt_map(self):
        ...
        self.assertIn(b"D1-D4 BELTS", response.data)
        self.assertIn(b"D34-D37 BELTS", response.data)
        self.assertIn(b"WHT/BLU", response.data)
        self.assertIn(b"BLU/BLK", response.data)
        self.assertEqual(response.data.count(b'class="neoermac-runout-card"'), 12)
        self.assertIn(b"neoermac-door-context", response.data)
        self.assertIn(b"D1", response.data)
        self.assertIn(b"D37", response.data)
        self.assertIn(b"EAST WHT/BLU BELT", response.data)
        self.assertIn(b"EAST ORG BELT", response.data)
        self.assertIn(b"WEST WHT/BLU BELT", response.data)
        self.assertIn(b"WEST ORG BELT", response.data)
