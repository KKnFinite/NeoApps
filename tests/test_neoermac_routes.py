...
class NeoErmacRoutesTest(unittest.TestCase):
    ...
    def test_authenticated_user_with_neoermac_access_can_access_menu(self):
        ...
        self.assertIn(b'neoermac_dashboard_mobile.webp', response.data)
    ...
    def test_neoermac_mobile_dashboard_css_hides_duplicate_body_title(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'class="ermac-command"', response.data)
        self.assertIn(b'<h1 class="visually-hidden">NeoErmac Dashboard</h1>', response.data)
        self.assertNotIn(b'neoermac-dashboard-hero', response.data)
        self.assertIn(b'<source media="(max-width: 900px)"', response.data)
        self.assertIn(b"data-neoermac-mobile-dashboard", response.data)
    ...
    def test_neoermac_dashboard_cards_use_dark_red_with_small_green_accent(self):
        ...
        self.assertIn(".ermac-command-tile {", css)
        self.assertIn("rgba(var(--node-ermac-primary-rgb), .5)", css)
        self.assertIn(".ermac-command-tile::before", css)
        ermac_css = Path('app/static/css/01-neoermac.css').read_text()
        self.assertNotIn('--node-sektor-', ermac_css)
        self.assertIn('background: var(--node-ermac-secondary)', ermac_css)
        self.assertIn('grid-template-columns: repeat(3, minmax(0, 1fr))', ermac_css)
    ...
    def test_door_view_outbound_destination_cards_have_prominent_scan_markup(self):
        ...
        self.assertIn(b"css/neoermac_door_workspace.css", response.data)
        self.assertNotIn(b">STAFFING</button>", response.data)
        self.assertIn(b"data-door-pull-form", response.data)
        self.assertIn(b"data-no-pull-toggle", response.data)
    ...
    def test_view_outbound_sorts_by_planned_pull_time_and_handles_missing_data(self):
        ...
        mobile_html = response.data.split(b'data-neoermac-outbound-mobile-list', 1)[1]
        self.assertIn(b'is-missing-mission', mobile_html)
