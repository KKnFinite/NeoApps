...
class NeoErmacRoutesTest(unittest.TestCase):
    ...
    def test_building_lineup_page_renders_runout_list(self):
        ...
        self.assertIn(b"DOOR 1", response.data)
        self.assertIn(b"SPECIAL RUNOUT", response.data)
        self.assertIn(b"RUNOUT SPAN", response.data)
        self.assertIn(b"DOOR 23", response.data)
