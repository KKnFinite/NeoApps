...
class MotherBrainRoutesTest(unittest.TestCase):
    ...
    def test_motherbrain_dashboard_renders_tile_menu(self):
        ...
        self.assertIn(b"data-motherbrain-dashboard-body", dashboard_body)
    ...
    def test_motherbrain_dashboard_tiles_include_expected_pages(self):
        ...
        self.assertIn(b"data-motherbrain-dashboard", dashboard_body)
        self.assertIn(b"data-motherbrain-dashboard-body", dashboard_body)
        self.assertGreater(len(dashboard_body.strip()), 1000)
        expected_tiles = (
            (b"manage-sort", b"MANAGE SORT"),
            (b"arrival-planning", b"ARRIVAL PLANNING"),
        ...
        for tile_key, label in expected_tiles:
            ...
            self.assertIn(b'data-motherbrain-dashboard-tile="' + tile_key + b'"', dashboard_body)
            self.assertIn(label, dashboard_body)
        self.assertTrue(document(response).one(**{"data-operational-mobile-header":None}).findall(cls="motherbrain-alert-tray"))
        ...
        self.assertIn(
            "body.motherbrain-home-page:not(.mobile-app-chrome) .motherbrain-dashboard-card",
            css,
        )
        self.assertIn("aspect-ratio: 1 / 1;", css)

    def test_motherbrain_header_navigation_routes_work(self):
        for path in ('/motherbrain/master-schedule', '/motherbrain/manage-sort', '/motherbrain/system-settings'):
            with self.subTest(path=path):
                response = self.client.get(path)
                root, drawer, dock = assert_mobile_drawer(self, response)
                sidebar = root.one(**{'data-motherbrain-desktop-side-nav':None})
                self.assertEqual(sidebar.one('img').attrs['src'], '/static/images/logos/newlogo_motherbrain.png')
                self.assertTrue(sidebar.findall('a', href=path, **{'aria-current':'page'}))
                self.assertEqual(len(root.findall(**{'data-operational-topbar':None})), 1)
                menu = drawer.one(**{'data-drawer-view':'menu'})
                for label in ('Manage Sort', 'Arrival Planning', 'Departure Planning', 'Parking Plan',
                              'Parking Rules', 'Master Schedule', 'Unmatched Queue', 'System Settings'):
                    self.assertIn(label, menu.text)
                self.assertIn('NeoPortal', menu.text)
                self.assertIn('Change Characters', root.text)
                self.assertTrue(root.findall(**{'data-motherbrain-alert-tray':None}))
                self.assertEqual(menu.one('form', action='/logout').attrs['method'], 'post')
        for path in ('/portal/manage', '/rfd', '/motherbrain'):
            self.assertEqual(self.client.get(path).status_code, 200)
        self.assertEqual(self.client.post('/logout').status_code, 302)
        self.assertEqual(self.client.get('/motherbrain').status_code, 302)
    ...
    def test_motherbrain_desktop_titles_match_side_menu_labels(self):
        ...
        for path, label, has_duplicate_title in pages:
            with self.subTest(path=path):
                ...
                sidebar = root.one(**{'data-motherbrain-desktop-side-nav':None})
                self.assertEqual(root.one(**{'data-operational-mobile-header':None}).one('small').text, label)
                active = sidebar.one('nav', 'motherbrain-desktop-side-menu').findall('a', **{'aria-current':'page'})
                self.assertEqual([link.text.strip() for link in active], [label])
                self.assertIn(label, sidebar.one('nav', 'motherbrain-desktop-side-menu').text)
    ...
    def test_kessler_grandmaster_can_access_motherbrain_pages(self):
        ...
        for path in get_paths:
            with self.subTest(path=path):
                ...
                self.assertIn(b'class="operational-mobile-identity"', response.data)
                ...
                self.assertIn(b'newlogo_motherbrain.png', desktop_sidebar)
    ...
    def test_add_alp_arrival_planning_row_allows_blank_wave_and_stores_null(self):
        operation = self._operation(sort_date=date(2026, 6, 24))
        db.session.add(operation)
        db.session.commit()
    ...
    def test_add_alp_arrival_planning_row_persists_selected_wave(self):
        for index, wave in enumerate(("1", "2"), start=1):
            operation = self._operation(sort_date=date(2026, 6, 24 + index))
            db.session.add(operation)
            db.session.commit()
            ...
            response = self.client.post(
                ...
                data={
                    ...
                    "flight_number": f"UPS99{index:02d}",
                    "airport": "SDF",
                    "tail_number": f"N99{index}UP",
                    "utc_datetime": "2026-06-24T07:24:00",
                    "reason": "No current operation mission match.",
                    "wave": wave,
                },
            ...
            self.assertEqual(response.status_code, 302)
            self.assertEqual(mission.wave, wave)

    def test_add_alp_arrival_planning_row_rejects_invalid_wave(self):
        operation = self._operation(sort_date=date(2026, 6, 24))
        db.session.add(operation)
        db.session.commit()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"Wave must be 1 or 2.", response.data)
        self.assertEqual(SortDateMission.query.count(), 0)

    def test_add_api_arrival_planning_row_allows_blank_wave_and_stores_null(self):
        operation = self._operation(sort_date=date(2026, 6, 24))
        db.session.add(operation)
        db.session.flush()
        ...
        response = self.client.post(
            ...
            data={"mission_type": "arrival", "wave": ""},
            follow_redirects=False,
        )

        db.session.refresh(item)
        mission = item.accepted_mission
        self.assertEqual(response.status_code, 302)
        self.assertIsNotNone(mission)
        self.assertIsNone(mission.wave)

    def test_arrival_planning_review_wave_field_is_not_required(self):
        operation = self._operation(sort_date=date(2026, 6, 24))
        db.session.add(operation)
        db.session.flush()
        item = self._api_review_item(
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('<select name="wave" aria-label="Wave">', add_form)
        self.assertNotIn('<select name="wave" required', add_form)
    ...
    def test_planning_review_preselects_inferred_wave(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('<select name="wave" required aria-label="Wave">', add_form)
    ...
    def test_mobile_arrival_planning_renders_simple_current_sort_list(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("<small>Arrival Planning</small>", html)
        ...
        self.assertNotIn("planning-mobile-arrival-wave", mobile_list)
        self.assertNotIn("planning-mobile-arrival-parking", mobile_list)
        self.assertNotIn("planning-mobile-arrival-status", mobile_list)
        self.assertNotIn("A01", mobile_list)
        self.assertNotIn("Expected", mobile_list)
    ...
    def test_mobile_departure_planning_renders_simple_current_sort_list(self):
        ...
        self.assertIn('<small>Departure Planning</small>', html)
    ...
    def test_parking_rules_page_renders_and_persists_settings(self):
        ...
        self.assertIn(b"value=\"7\"", reload_response.data)
    ...
    def test_parking_rules_page_renders_and_persists_building_lineup_belt_preferences(self):
        ...
        section_html = self._html_section(
            response.data.decode("utf-8"),
            "parking-rule-section-building-lineup-belt-preferences",
        )
        self.assertNotIn(">ACTION<", section_html)
        ...
        save_response = self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data=self._belt_preferences_bulk_post_data(
                operation,
                {
                    "4/6": ["A", "B"],
                    "17/21": ["C", "D"],
        ...
        self.assertEqual(save_response.status_code, 302)
        self.assertTrue(
            save_response.location.endswith("#parking-rule-section-building-lineup-belt-preferences")
        )
        ...
        self.assertIn(b'name="building_lineup_belt_preference_17_21" value="C" checked', reload_response.data)
        self.assertIn(b'name="building_lineup_belt_preference_17_21" value="D" checked', reload_response.data)

    def test_parking_rules_belt_preference_bulk_save_reload_update_and_clear(self):
        operation = self._parking_operation()
        db.session.commit()

        first_save = self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data=self._belt_preferences_bulk_post_data(operation, {"4/6": ["A"]}),
        )
        ...
        second_save = self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data=self._belt_preferences_bulk_post_data(operation, {"4/6": ["A", "B"]}),
        )
        ...
        update_save = self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data=self._belt_preferences_bulk_post_data(operation, {"4/6": ["C"]}),
        )
        ...
        clear_save = self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data=self._belt_preferences_bulk_post_data(operation, {}),
        )
    ...
    def test_parking_rules_belt_preference_bulk_save_clears_complete_grid(self):
        operation = self._parking_operation()
        self._belt_preference("4/6", "A")
        self._belt_preference("6/9", "B")
        ...
        preferences = active_belt_pair_preference_map(self.rfd_gateway)
        for pair in BUILDING_LINEUP_PARKING_BELT_PAIRS:
            self.assertEqual(preferences[pair["pair_key"]], ())

    def test_parking_rules_belt_preference_bulk_save_preserves_other_belt_pairs(self):
        operation = self._parking_operation()
    ...
    def test_parking_rules_belt_preference_bulk_save_isolates_other_parking_rules(self):
        operation = self._parking_operation()
        settings = MotherBrainParkingSettings(
            ...
            deice_spacing_threshold_minutes=15,
            inbound_same_ramp_spacing_minutes=5,
            preferred_max_per_ramp=None,
        )
        existing_rule = self._parking_rule(
            BLOCKED_PARKING_POSITION,
            "position",
            "D01",
            "D",
        ...
        data.update(
            {
                "include_remote_default": "1",
                "include_throat_default": "1",
                "deice_spacing_threshold_minutes": "99",
                "inbound_same_ramp_spacing_minutes": "88",
                "preferred_max_per_ramp": "77",
                "rule_ids": [str(existing_rule.id)],
                f"delete_rule_{existing_rule.id}": "1",
                "new_aircraft_type_ramp_restriction_subject": "767",
                "new_aircraft_type_ramp_restriction_ramp": "R",
            }
        )
        ...
        reloaded_settings = MotherBrainParkingSettings.query.filter_by(
            gateway_id=self.rfd_gateway.id,
        ).one()
        self.assertFalse(reloaded_settings.include_remote_default)
        self.assertFalse(reloaded_settings.include_throat_default)
        self.assertEqual(reloaded_settings.deice_spacing_threshold_minutes, 15)
        self.assertEqual(reloaded_settings.inbound_same_ramp_spacing_minutes, 5)
        self.assertIsNone(reloaded_settings.preferred_max_per_ramp)
        self.assertIsNotNone(db.session.get(MotherBrainParkingRule, existing_rule.id))
        self.assertEqual(
            MotherBrainParkingRule.query.filter_by(
                gateway_id=self.rfd_gateway.id,
                rule_category=AIRCRAFT_TYPE_RAMP_RESTRICTION,
                subject_value="767",
                ramp_code="R",
            ).count(),
            0,
    ...
    def test_parking_rules_belt_preference_rejects_invalid_pair_and_ignores_invalid_ramps(self):
        ...
        invalid_pair_data = self._belt_preferences_bulk_post_data(operation, {"6/9": ["D"]})
        invalid_pair_data["building_lineup_belt_preference_2_5"] = ["A"]
        ...
        invalid_ramp_response = self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data={
                **self._belt_preferences_bulk_post_data(operation, {"6/9": ["D"]}),
    ...
    def test_parking_rules_belt_preference_empty_and_invalid_values_do_not_save(self):
        ...
        self.client.post(
            f"/motherbrain/parking-rules?operation_id={operation.id}",
            data={
                **self._belt_preferences_bulk_post_data(operation),
                "building_lineup_belt_preference_4_6": ["Z"],
    ...
    def test_parking_rules_edit_permission_is_enforced(self):
        ...
        self.assertNotIn(b"SAVE BUILDING LINEUP PREFERENCES", get_response.data)
        ...
        bulk_post_response = self.client.post(
            "/motherbrain/parking-rules",
            data={
                "building_lineup_belt_preferences_present": "1",
                "save_building_lineup_belt_preferences": "1",
                "building_lineup_belt_preference_4_6": ["A", "B"],
            },
        )

        self.assertEqual(bulk_post_response.status_code, 302)
        self.assertEqual(MotherBrainParkingRule.query.count(), 0)

    def test_parking_rules_edit_permission_defaults_to_simulator(self):
        ensure_default_permission_rules()
        edit_rule = PermissionRule.query.filter_by(
            permission_key="motherbrain.parking_rules.edit",
        ).one()
    ...
    def test_parking_validator_allows_manual_normal_bank_out_of_fill_order(self):
        ...
        self._parking_assignment(operation, "N458UP", "D03")
        self._parking_assignment(operation, "N459UP", "D06")
        db.session.commit()
        ...
        self.assertNotIn("D03 cannot be used until D01, D02 are filled.", messages)
    ...
    def test_parking_validator_echo_767_positions_do_not_use_two_slot_footprint(self):
        operation = self._parking_operation()
        self._parking_pair(operation, "N901UP", aircraft_type="757", destination="PHL")
        ...
        db.session.flush()
        self._parking_assignment(operation, "N901UP", "E01", ramp_code="E")
        ...
        self.assertNotIn("E02 is blocked by 767 parked at E01.", messages)
    ...
    def test_parking_validator_allows_767_anchors_except_04_and_08(self):
        ...
        for tail, position in (
            ...
            self._parking_pair(operation, tail, aircraft_type="757", destination=position)
            db.session.flush()
            self._parking_assignment(operation, tail, position, ramp_code=position[0])
        db.session.commit()
        tail_rows = parking_plan_context(self.rfd_gateway, operation=operation)["tail_rows"]
        ...
        self.assertNotIn(
            "767 at B06 is invalid because 767 aircraft cannot anchor at 06.",
            messages,
        )
        self.assertNotIn(
            "767 at C07 is invalid because 767 aircraft cannot anchor at 07.",
            messages,
        )
        self.assertIn(
            "767 at D04 is invalid because 767 aircraft cannot anchor at 04.",
            messages,
        )
        self.assertIn(
            "767 at D08 is invalid because 767 aircraft cannot anchor at 08.",
            messages,
        )
    ...
    def test_parking_optimizer_spreads_close_inbound_etas_across_ramps(self):
        operation = self._parking_operation()
        db.session.add(
            MotherBrainParkingSettings(
                gateway_id=self.rfd_gateway.id,
        ...
        self._parking_pair(
            operation,
            "N411UP",
            arrival_local=datetime(2026, 6, 18, 23, 2),
            departure_local=datetime(2026, 6, 19, 2, 20),
            destination="SDF",
        )
        self._parking_rule(
            AIRCRAFT_TYPE_RAMP_PREFERENCE,
        ...
        self.assertEqual(preview["runtime_toggles"]["inbound_same_ramp_spacing_minutes"], 5)
        self.assertEqual(preview["runtime_toggles"]["inbound_spacing_status"], "applied")
        self.assertNotEqual(
            suggestions["N410UP"]["position"][:1],
            suggestions["N411UP"]["position"][:1],
        )
        self.assertIn("Inbound ETA spacing checked", reasons)

    def test_parking_optimizer_allows_close_inbound_same_ramp_when_unavoidable(self):
        operation = self._parking_operation()
        db.session.add(
            MotherBrainParkingSettings(
                gateway_id=self.rfd_gateway.id,
        ...
        for index in range(2):
            self._parking_pair(
                operation,
                f"N42{index}UP",
                arrival_local=datetime(2026, 6, 18, 23, 0) + timedelta(minutes=index * 2),
    ...
    def test_parking_optimizer_prefers_normal_ramp_side_balance(self):
        operation = self._parking_operation()
        db.session.add(
            MotherBrainParkingSettings(
                gateway_id=self.rfd_gateway.id,
    ...
    def test_parking_optimizer_allows_side_imbalance_when_unavoidable(self):
        ...
        self._parking_rule(ORIGIN_RAMP_PREFERENCE, "origin", "ONT", "A", behavior="required")
        for position in ("A05", "A06", "A07", "A08"):
            self._parking_rule(
                BLOCKED_PARKING_POSITION,
                "position",
                position,
        ...
        self.assertTrue(all(position in {"A01", "A02", "A03"} for position in positions))
    ...
    def test_parking_optimizer_origin_ramp_requirement_is_hard(self):
        operation = self._parking_operation()
        self._parking_pair(operation, "N457UP", origin="SDF", aircraft_type="757")
        self._parking_pair(operation, "N349UP", origin="ONT", aircraft_type="757", destination="SDF")
        self._parking_rule(
            ORIGIN_RAMP_PREFERENCE,
            "origin",
            "SDF",
        ...
        self.assertEqual(suggestions["N457UP"]["position"][:1], "B")
    ...
    def _belt_preferences_bulk_post_data(self, operation, preferences=None):
        data = {
            "operation_id": str(operation.id),
            "return_to": "parking-rule-section-building-lineup-belt-preferences",
            "building_lineup_belt_preferences_present": "1",
            "save_building_lineup_belt_preferences": "1",
        }
        for pair_key, ramps in (preferences or {}).items():
            if ramps:
                data[parking_belt_preference_form_field(pair_key)] = list(ramps)
        return data

    def _html_section(self, html, section_id):
        marker = f'id="{section_id}"'
        self.assertIn(marker, html)
        marker_index = html.index(marker)
        start_index = html.rfind("<section", 0, marker_index)
        end_index = html.find("</section>", marker_index)
        self.assertNotEqual(start_index, -1)
        self.assertNotEqual(end_index, -1)
        return html[start_index : end_index + len("</section>")]
